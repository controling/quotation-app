...
def list_items(params: ListParams, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    if params.search:
        ...
        q = q.filter(or_(DrugItem.name.like(like), DrugItem.category.like(like), DrugItem.standard.like(like)))
...
@router.post("/update")
def update_item(data: DrugItemUpdate, item_id: int = 0, db: Session = Depends(get_db), admin=Depends(require_admin)):
