    ...
    <div v-show="currentStep === 1">
      <div class="m-card">
        <div class="m-card-header">
          <h3>选择样品</h3>
          <span style="font-size:12px;color:var(--text-3)">共 {{ sampleTotal }} 个样品</span>
        </div>
...
onMounted(() => { loadSamples(1) })

async function loadSamples(page, append = false) {
  if (sampleLoading.value) return
  sampleLoading.value = true
  try {
    const { data } = await drugItemsApi.list({ search: sampleSearch.value, page, page_size: 20 })
    const newCats = (data.samples || []).map(s => s.category)
    // Merge items into store for popup display
...
function loadMoreSamples() {
  loadSamples(sampleCurrentPage + 1, true)
}
