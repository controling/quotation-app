  ...
  <div class="page-content">
    ...
    <template v-else>
      ...
      <div class="m-search"  style="top: -14px;">
        <div class="m-search-bar">
          ...
          <input type="text" placeholder="搜索包材名称、检测项目..." v-model="searchText" @input="onSearch" />
        </div>
      </div>
      ...
      <div style="font-size:12px;color:var(--text-3);padding:0 0 8px">
        共 {{ filteredGroups.length }} / {{ totalSamples }} 个样品, {{ totalPkgItems }} 项检测
      </div>
    ...
    <van-dialog v-model:show="showDialog" :title="editingItem ? '编辑检测项目' : '新增检测项目'" show-cancel-button @confirm="onSave" :before-close="beforeClose">
      <div style="padding: 12px 16px">
        <van-field v-model="formData.category" label="样品名称" placeholder="如：玻璃类药包材" />
        <van-field v-model="formData.name" label="检测项目" placeholder="如：外观" />
        <van-field v-model="formData.standard" label="标准" placeholder="如：YBB00032005-2015" />
        <van-field v-model="formData.method" label="方法" placeholder="如：目视法" />
...
onMounted(async () => {
  ...
  try {
    ...
    totalPkgItems.value = res.data.packaging_items
    totalSamples.value = res.data.packaging_samples || res.data.packaging_items
  } catch {}
...
async function onSave() {
  try {
    ...
    if (editingItem.value) {
      await packagingItemsApi.update({ item_id: editingItem.value.id, ...payload })
      toast('更新成功')
    } else {
      await packagingItemsApi.create(payload)
      toast('添加成功')
    }
    store.loaded = false
    store.loadAll()
...
async function onImport(items) {
  try {
    ...
    const { data } = await packagingItemsApi.import(payload)
